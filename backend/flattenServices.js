...
// Flatten and add serviceType to each item
const flattenedServices = [];

for (const [serviceType, services] of Object.entries(servicesData)) {
  services.forEach(service => {
    flattenedServices.push({ ...service, serviceType });
  });
}

// Write the flattened data to a JSON file
fs.writeFile("./flattenedServices.json", JSON.stringify(flattenedServices, null, 2), (err) => {
  if (err) {
    console.error("Error writing file:", err);
  } else {
    console.log("flattenedServices.json file created successfully!");
  }
});
